...
SkeletonBone::SkeletonBone ( const Vector4& inPosition, const _NUI_SKELETON_BONE_ORIENTATION& orient, const NUI_SKELETON_POSITION_TRACKING_STATE& trackingState, ofVec4f g) {
	...
	{	
		...
		for ( int i = 0; i < NUI_SKELETON_COUNT; i++ ) 
		{
			...
			if (  k4wSkeletons.SkeletonData[ i ].eTrackingState == NUI_SKELETON_TRACKED || k4wSkeletons.SkeletonData[ i ].eTrackingState == NUI_SKELETON_POSITION_ONLY ) {
				...
				{
					ofVec4f g(k4wSkeletons.vFloorClipPlane.x, k4wSkeletons.vFloorClipPlane.y, k4wSkeletons.vFloorClipPlane.z, k4wSkeletons.vFloorClipPlane.w);
					SkeletonBone bone( k4wSkeletons.SkeletonData[i].SkeletonPositions[j], bones[j], k4wSkeletons.SkeletonData[i].eSkeletonPositionTrackingState[j], g );
